...
function train {
  ...
  [ -z "$ds_t" ]  && ds_t="spaced_api_call"

  [ -z "$ds_format" ]  && ds_format="FB"

  [ -z "$response_type" ]  && response_type="single_bot_response"
  ...
  python single_dialog.py --task_id=$task --train=True --hops=$hops --ds_name=$ds_name --task_size=$task_size \
...
function evaluate {
  ...
  python single_dialog.py --task_id=$task --OOV=$OOV --train=False --hops=$hops --ds_name=$ds_name --task_size=$task_size \
--ds_type=$ds_t --order_info=$order_info --ds_format=$ds_format --response_type=$response_type \
