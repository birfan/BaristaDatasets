...
tf.flags.DEFINE_integer("epochs", 100, "Number of epochs to train for.") # this was 250 for split-memory, but it was a lot
tf.flags.DEFINE_integer("embedding_size", 20, "Embedding size for embedding matrices.")
tf.flags.DEFINE_integer("memory_size", 250, "Maximum size of memory.") #TODO: CHECK THIS!
...
# tf.flags.DEFINE_string("data_dir", "../../../data/personalized-dialog-dataset/small/", "Directory containing personalized dialog tasks")
...
class chatBot(object):
    def __init__(self, data_dir, model_dir, vocab_dir, task_id, 
        ...
        profile_size = None
        if "personalised" in ds_name:
            profile_size = 3 # three profile attributes: isCusKnown, cusID , cusName 
            if self.order_info:
                profile_size = 6 # six profile attributes: isCusKnown, cusID , cusName , prefSize , prefDrink , prefExtra 
        ...
        self.trainData, self.testData, self.valData = load_dialog_task(
            self.task_dir, self.task_id, self.candid2indx, self.OOV, ds_name, profile_size=profile_size)
        ...
        self.model = MemN2NDialog(self.batch_size, self.vocab_size, self.n_cand, 
                                  ...
                                  optimizer=optimizer, name='PersonalisedMemN2N', task_id=task_id)
...
#                 summary_writer.flush()
    ...
    def test(self):
        ...
        for num_pred in range(len(test_preds)):
            ...
            prof = ""
            for p in testP[num_pred]:
                decoded = convert_word_index_to_sentence(p, self.vocab) 
            ...
            if self.log_predictions:
                self.id_example = log_predictions_to_file(contex, self.indx2candid[test_preds[num_pred]], self.indx2candid[answer], profile=prof, id_example=self.id_example, incorrect_pred_path=self.incorrect_pred_path, correct_pred_path=self.correct_pred_path)
