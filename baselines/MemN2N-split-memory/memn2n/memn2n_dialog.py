...
class MemN2NDialog(object):
    ...
    def __init__(self, batch_size, vocab_size, candidates_size, 
                 ...
                 optimizer=tf.train.AdamOptimizer(learning_rate=1e-2), #TODO: try with gradient descent
                 session=tf.Session(),
                 name='MemN2N',
                 task_id=1):
        ...
        cross_entropy = tf.nn.sparse_softmax_cross_entropy_with_logits(
            labels=self._answers, logits=logits, name="cross_entropy")
        ...
        predict_proba_op = tf.nn.softmax(logits, name="predict_proba_op") # TODO: LOOK AT THIS ONE TO SEE IF IT WORKS BETTER
    ...
    def _inference(self, profile, stories, queries):
        ...
        with tf.variable_scope(self._name):
            ...
            #TODO: Understand how memories are separated into two memory parts (compare with normal memn2n)
            #TODO: need to have multiple memories 
...
#         print("stories:", np.shape(stories))
...
#         print("answers:", np.shape(answers))
    ...
    def predict(self, profile, stories, queries):
        ...
        Args:
            profile: Tensor (None, sentence_size)
            stories: Tensor (None, memory_size, sentence_size)
            queries: Tensor (None, sentence_size)
