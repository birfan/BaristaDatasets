...
from parlai_internal.scripts.utils import log_predictions_to_file, update_predictions_path, modelzoo_path
...
class Seq2seqAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        #BI: additional arguments for logging predictions
...
class PersonachatSeqseqAgentBasic(Seq2seqAgent):
    ...
    def observe(self, obs):
        ...
        if len(observation) == 1 and 'episode_done' in observation:
            ...
            if self.usepersona:
                #BI: changed to use profile information instead of persona
                for t in text_split:
                    if t.startswith('False') or t.startswith('false') or t.startswith('True') or t.startswith('true'):
                        self.persona_given = '' # overwrite the profile if it is updated
                        attribs = t.split(' , ')
...
class PersonachatSeqseqAgentSplit(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        agent.add_argument('--interactive-mode', type=bool, default=False,
            help='helps print nicer text during interactive mode')
        agent.add_argument('--use-persona', type=bool, default=False,
            help='if task is personalised-barista, this should be True')
    ...
    def observe(self, obs):
        ...
        if len(observation) == 1 and 'episode_done' in observation:
           ...
           return observation
            ...
            self.last_obs = observation['text']
            self.episode_done = observation['episode_done']
            observation.force_set('text', self.prev_dialog + observation['text'])
    ...
    def _score_candidates(self, cands, xe, encoder_output_persona, hidden_persona, hidden, attn_mask):
        ...
        #TODO: CODE EXPLODES HERE IN CPU!
        cands_attn_mask = (
            attn_mask.contiguous()
            .view(sz[0], 1, sz[1])
            .expand(sz[0], cands.size(1), sz[1])
    ...
    def batch_act(self, observations):
        ...
        if self.opt['datatype'] in ['valid', 'test'] and self.opt['personachat_interact']:
            ...
            for msg in self.teacher.data_dialogs['train']['messages']:
                f1_tmp = F1Metric.compute(' '.join(predictions[0]), [msg[1][1]])
