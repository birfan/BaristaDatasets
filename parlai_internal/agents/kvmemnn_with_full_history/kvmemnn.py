...
from parlai_internal.scripts.utils import log_predictions_to_file, update_predictions_path
...
def maintain_dialog_history(history, observation, reply='',
                            historyLength=250, useReplies="labels",
    ...
    # BI: Better performance with this in barista-personalised as well
    # history['dialog'].clear()
    ...
    if 'text' in observation:
        ...
        for txt in txts:
            # BI: CHANGED THIS SECTION FOR PROFILE IN PERSONALISED BARISTA DATASET
            if useProfile and (txt.startswith('False') or txt.startswith('false') or txt.startswith('True') or txt.startswith('true')):
                history['profile'] = [] # overwrite the profile if it is updated
                attribs = txt.split(' , ')
                for attrib in attribs:
                    history['profile'].append(Variable(torch.LongTensor(parse(attrib)).unsqueeze(0)))
            else:
                utt = parse(txt)
    ...
    return history['dialog'], history['profile']
...
class KvmemnnAgent(Agent):
    ...
    def add_cmdline_args(argparser):
        ...
        agent.add_argument('--calculate-fix-cands', type='bool', default=False,
                           help='Use the candidates calculated for the first validation (not from file).')
        agent.add_argument('--log-predictions', type='bool', default=False,
                           help='Log predictions to json or txt file (depending on the extension of the dupi or dupc). '
                           'Needs -dupi or -dupc to be filled.')
        agent.add_argument('-dupi','--dump-incorrect-predictions-path', type=str, default=None,
                           help='Dump incorrect predictions into the specified path.'
                           'Needs to start with zoo: or izoo: if not None (default).')
        agent.add_argument('-dupc','--dump-correct-predictions-path', type=str, default=None,
                           help='Dump correct predictions into the specified path.'
                           'Needs to start with zoo: or izoo: if not None (default).')
    ...
    def reset(self):
        ...
        self.cands_done = []
        self.history = {}
        self.calc_fix_cands = None
        self.calc_fix_cands_txt = None
        self.calc_fix_cands_enc = None
        # set up optimizer
    ...
    def predict(self, xs, ys=None, cands=None, cands_txt=None, obs=None):
        ...
        if is_training: #
            ...
            if len(negs) > 0:
                ...
                #BI: added calculation of training candidates to evaluate training accuracy 
                cands_train = []
                cands_train.append(self.v2t(ys))
                for neg in negs:
                    cands_train.append(self.v2t(neg))
                # print("query", self.v2t(xs))
                train_val,train_ind=pred.sort(descending=True)
                ypred_train = cands_train[train_ind[0].item()] # reply to match
                # print("predicted", ypred_train)
                return [{'text':ypred_train, 'metrics':metrics}]
        else:
            fixed = False
            if hasattr(self, 'fixedCands') and self.fixedCands:
                ...
                self.tricks=False
            if not self.calculate_fix_cands and (cands is None or self.take_next_utt):
            ...
            if fixed:
                ...
                # print("   [correct response: " + self.v2t(ys) + "]")
...
#                     print("   [1st hop nextut: " + ypred + "]")
                ...
                if self.calculate_fix_cands:
                    #BI: added calc_fix_cands for calculating candidates at the first example of validation, and 
                    # using that value for faster validation
                    #BI: also added calc_fix_cands_enc for using the calculated encodings for calc_fix_cands for speed up
                    if self.calc_fix_cands_enc is None:
                        self.calc_fix_cands_enc = self.model.get_cands_encoded(self.calc_fix_cands)
                    if self.opt['loss'] == 'cosine':
                        xe, ye = self.model(xs, obs[0]['mem'], ys, self.calc_fix_cands, self.calc_fix_cands_enc)
                        pred = nn.CosineSimilarity().forward(xe,ye)
                    else:
                        x = self.model(xs, obs[0]['mem'], ys, self.calc_fix_cands, self.calc_fix_cands_enc)
                        pred = x #.squeeze()

                    val,ind=pred.sort(descending=True)
                    ypred = self.calc_fix_cands_txt[ind[0].item()] # match
                    tc = []
                    for i in range(min(100, ind.size(0))):
                        tc.append(self.calc_fix_cands_txt[ind[i].item()])
                else:            
                    if self.opt['loss'] == 'cosine':
                        xe, ye = self.model(xs, obs[0]['mem'], ys, cands[0])
                        pred = nn.CosineSimilarity().forward(xe,ye)
    ...
    def batchify(self, observations):
        ...
        if ys is None and self.calc_fix_cands is None:
            ...
            if self.calculate_fix_cands:
                self.calc_fix_cands = cands[0].copy()
                self.calc_fix_cands_txt = cands_txt[0].copy()
